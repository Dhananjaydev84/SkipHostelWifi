// ============================================================
// auth.js — Shared WiFi login logic (popup + background service worker)
//
...
//
// Exported via IIFE so it works in both contexts:
//   • popup page  → attaches to window.doLogin
//   • service worker (background.js) → attaches to self.doLogin
// ============================================================
(function (global) {
  ...
  async function doLogin(userId) {
    ...
    var timer = setTimeout(function () {
      controller.abort(); // triggers AbortError in the catch block below
    }, TIMEOUT_MS);
    ...
    try {
      ...
      var response = await fetch(POST_URL, {
        ...
        body:    formData,
        signal:  controller.signal  // links this fetch to the AbortController
      });
      ...
      // AbortError means the request hit the TIMEOUT_MS limit
  ...
  // ----------------------------------------------------------
  // Attach doLogin to the correct global scope:
  //   • service worker → self
  //   • browser page   → window
  //   • fallback       → this (legacy environments)
  // ----------------------------------------------------------
